(function ($) {
    ...
    function lynessa_form() {
        $('.showbtn').on('click', function (e) {
            $(this).parents('.dayuna-form-toggle').siblings('.dayuna-form-show').slideToggle();
            e.preventDefault();
        });
    }
    ...
    function lynessa_title_tooltip() {
        $(".dayuna-tooltip").mousemove(function (event) {
            var relX = event.pageX - $(this).offset().left - $(this).children('.dayuna-tooltip-inner').outerWidth() / 2;
            var relY = event.pageY - $(this).offset().top - $(this).children('.dayuna-tooltip-inner').outerHeight() - 5;
            $(this).children('.dayuna-tooltip-inner').css({"top": relY, "left": relX});
        });
    }
    ...
    $(window).on("load", function () {
        ...
        if ($('.dayuna-threed-view').length > 0) {
            lynessa_threesixty();
        }
        if ($('.vertical-menu').length > 0) {
            lynessa_auto_width_vertical_menu();
        }
        if ($('.dayuna-countdown').length) {
            lynessa_countdown($('.dayuna-countdown'));
        }
